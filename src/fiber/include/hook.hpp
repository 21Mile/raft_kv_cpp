...
#include <sys/types.h>    // 解决 ssize_t, socklen_t 等问题
#include <unistd.h>       // 解决 usleep 等相关函数声明
#include <sys/socket.h>   // 解决 sockaddr 等网络相关类型
#include <cstdint>        // 解决 uint64_t 等类型
#include <sys/ioctl.h>
#include <fcntl.h>       // 若同时用到 O_NONBLOCK
namespace monsoon
{
    // 当前线程是否hook
    bool is_hook_enable();
