...
// code
class KvServer : raftKVRpcProctoc::kvServiceRpc
{
private:
    std::mutex m_mtx;
    int m_me;
    std::shared_ptr<Raft> m_raftNode;               // 节点指针
    std::shared_ptr<LockQueue<ApplyMsg>> applyChan; // kvServer和节点通信的通道
    int m_maxRaftState;                             // snapshot if log grows this big
    std::string m_serializedKVData;                 // todo ： 序列化后的kv数据，理论上可以不用，但是目前没有找到特别好的替代方法
    SkipList<std::string, std::string> m_skipList;  // 存储数据的跳表
    ...
    std::unordered_map<std::string, int> m_lastRequestId; //记录 每个 ClientId 最近处理过的 RequestId。
...
public:
    ...
    void ExecuteGetOpOnKVDB(Op op, std::string *value, bool *exist); // 执行Put
    void ExecutePutOpOnKVDB(Op op);                                  // 执行Get
